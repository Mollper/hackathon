...
function RightSidebar() {
  ...
  useEffect(() => {
    // Загружаем статистику за последние 7 дней
    ...
    // Загружаем активные ивенты от админа
    supabase
      .from('alerts')
      .select('*')
      .eq('active', true)
  ...
  return (
    <aside className="hidden lg:block w-80 bg-gray-50 h-screen sticky top-0 p-6 overflow-y-auto transition-colors duration-300">
      ...
      {/* Сводка за неделю — реальные данные */}
      ...
      {/* Ивенты/предупреждения от админа */}
      ...
      {/* Кнопка админ-панели если роль admin */}
      {profile?.role === 'admin' && (
        <Link
          href="/admin"
