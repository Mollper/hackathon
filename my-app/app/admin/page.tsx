...
const STATUS_OPTIONS = [
  ...
  { value: 'rejected',    label: 'Отклонено',  color: 'bg-gray-100 text-gray-600' },
];
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8">
      ...
      <div className="flex gap-2 mb-6 bg-gray-100 p-1 rounded-2xl w-fit">
        <button
          ...
          className={`px-5 py-2 rounded-xl font-semibold text-sm transition ${tab === 'posts' ? 'bg-white shadow text-gray-900' : 'text-gray-500'}`}
        >
          📋 Посты ({posts.length})
        </button>
        <button
          onClick={() => setTab('alerts')}
          className={`px-5 py-2 rounded-xl font-semibold text-sm transition ${tab === 'alerts' ? 'bg-white shadow text-gray-900' : 'text-gray-500'}`}
        >
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            ...
            <div className="flex flex-col gap-3">
              <select
                ...
                className="border border-gray-200 rounded-xl p-3 outline-none focus:border-blue-500 bg-white text-sm"
              >
              ...
              <input
                ...
                className="border border-gray-200 rounded-xl p-3 outline-none focus:border-blue-500 text-sm"
                value={alertTitle}
                onChange={(e) => setAlertTitle(e.target.value)}
              />
              <textarea
                placeholder="Текст уведомления..."
                className="border border-gray-200 rounded-xl p-3 outline-none focus:border-blue-500 text-sm resize-none h-24"
                value={alertMessage}
                onChange={(e) => setAlertMessage(e.target.value)}
              />
              <button
                onClick={createAlert}
                disabled={saving || !alertTitle || !alertMessage}
                className="bg-gray-900 text-white font-bold py-3 rounded-xl hover:bg-gray-700 disabled:opacity-50 transition"
              >
              ...
              <div key={alert.id} className={`bg-white rounded-2xl p-5 shadow-sm border flex justify-between items-start gap-4 ${alert.active ? 'border-blue-100' : 'border-gray-100 opacity-60'}`}>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`p-2 rounded-xl transition ${alert.active ? 'bg-yellow-100 text-yellow-700 hover:bg-yellow-200' : 'bg-green-100 text-green-700 hover:bg-green-200'}`}
                    title={alert.active ? 'Скрыть' : 'Показать'}
                  >
